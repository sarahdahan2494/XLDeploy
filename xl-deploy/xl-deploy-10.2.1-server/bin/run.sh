...
if [ "$1" = "plugin-manager-cli" ]; then
    BOOTSTRAPPER="com.xebialabs.deployit.PluginManagerCliBootstrapper"
    shift
fi

# Run XL Deploy server
$JAVACMD -Dfile.encoding=UTF-8 $DEPLOYIT_SERVER_OPTS -classpath "${DEPLOYIT_SERVER_CLASSPATH}" $BOOTSTRAPPER "$@"
